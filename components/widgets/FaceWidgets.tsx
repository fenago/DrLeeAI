...
export function FaceWidgets({ onCalibrate }: FaceWidgetsProps) {
  ...
  // Include all 48 emotions from the Hume API
  ...
  return (
    <div>
      <div className="md:flex">
        ...
        />
        {!onCalibrate && (
          <div className="ml-10 overflow-y-auto max-h-[500px] pr-4" style={{ scrollbarWidth: 'thin' }}>
            {/* Pre-compute the scaled emotions once instead of in each Loader component */}
            {useMemo(() => {
              // Only render emotion components when we have emotions to show
              if (emotions.length === 0) return null;
              
              // Compute scaled emotions once for all emotion components
              const scaledEmotions = scaleEmotionsToRanges(emotions);
              
              return (
                <>
                  <TopEmotions emotions={scaledEmotions} />
                  <Descriptor className="mt-4" emotions={scaledEmotions} />
                  <div className="mt-6">
            ...
            }, [emotions, loaderNames, numLoaderLevels])}
